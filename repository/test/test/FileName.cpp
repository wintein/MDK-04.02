...
void change_array(double** matrix, int n) {
    for (int i = 0; i < n - 1; i++)
    {
...
void delete_array(double** matrix, int n) {
    for (int i = 0; i < n; i++) {
        delete[]matrix[i];
    }
    delete[]matrix; //Освобождение памяти, выделенной под массив указателей
    matrix = NULL; //Очистка указателя
}
...
int main()
{
    ...
    change_array(matrix, n);
    cout << "------------Измененная матрица:------------" << endl;
    print_array(matrix, n);
    delete_array(matrix, n);
    return 0;
}